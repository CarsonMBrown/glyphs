...
SAVE_PATH = os.path.join("weights", "cnn")
...
def train_model(lang_file, annotations_file, training_data_path, validation_data_path, model_class, *, epochs=50,
                batch_size=8, num_workers=1, resume=False, start_epoch=0):
    # Load validation set for model init, not loading train set yet
    validation_set = ImageLoader(lang_file, annotations_file, validation_data_path, transform=preprocess)
    validation_loader = torch.utils.data.DataLoader(validation_set, batch_size=batch_size, num_workers=num_workers)

    model = model_class(24).cuda()

    model_path = os.path.join(SAVE_PATH, model.get_name())
    if not os.path.exists(model_path):
        os.mkdir(model_path)
    save_file = os.path.join(model_path, "rnn_" + str(start_epoch) + ".pt")
    ...
    training_set = ImageLoader(lang_file, annotations_file, training_data_path, transform=preprocess)
